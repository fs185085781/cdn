(function(){
    ...
    * */
    ...
    window.initVersion=function(vc){
        ...
        if(ieProp.ltIE9){
            /*兼容IE6,IE7,IE8不支持axios的情况*/
            utils.syncLoadData(utils.uihost+"/plugins/jquery/jquery.min.js",function(text){eval(text);});
            document.write('<script src="'+utils.uihost+'/plugins/axios/jquery-axios.js?jsv='+version+'"></script>');
        }else{
            document.write('<script src="'+utils.uihost+'/plugins/axios/axios.js?jsv='+version+'"></script>');
        };
