(function () {
    ...
    var miniUpdate = function(e){
        var that = e.sender;
        if(that.showPager && exportMap[that.uid].exportStatus == "data"){
            if(exportMap[that.uid].currentIndex != exportMap[that.uid].maxIndex){
                exportMap[that.uid].currentIndex += 1;
                that.set({pageIndex:exportMap[that.uid].currentIndex});
                ...
                exportMap[that.uid].index = 0;
                mini.hideMessageBox(exportMap[that.uid].loadingId);
                exportMap[that.uid].loadingId = mini.loading("正在生成第"+(exportMap[that.uid].currentIndex+1)+"页数据,共"+(exportMap[that.uid].maxIndex+1)+"页");
                that.reload();
                return;
            }
        }
        if(exportMap[that.uid].exportStatus == "data"){
            if(!exportMap[that.uid].exportDownStartTime){
                ...
                console.log("总共耗时:"+(now-exportMap[that.uid].exportInitStartTime)+"ms");
            }
            mini.hideMessageBox(exportMap[that.uid].loadingId);
    ...
    mini.DataGrid.prototype[createColumnsHtmlMethodKey] = function(a,b,c){
        ...
        if(exportMap[that.uid] && exportMap[that.uid].exportStatus == "init"){
            if(a && a.length>0){
                ...
                if(that.showPager){
                    if(exportMap[that.uid].currentIndex != 0){
                        exportMap[that.uid].exportTableStr = "";
                    }
                    var mergeColumnsMethod = exportMap[that.uid].mergeColumnsMethod;
                    if(mergeColumnsMethod){
    ...
    mini.DataGrid.prototype[createRowsHtmlMethodKey] = function(a,b,c,d,e){
        var that = this;
        if(exportMap[that.uid] && (exportMap[that.uid].exportStatus == "column" || exportMap[that.uid].exportStatus == "data")){
            if(exportMap[that.uid].exportStatus == "column"){
                exportMap[that.uid].exportStatus = "data";
            }
            if(!exportMap[that.uid].exportRowStartTime){
                var now = new Date().getTime();
                exportMap[that.uid].exportRowStartTime = now;
                console.log("头生成耗时:"+(now-exportMap[that.uid].exportColumnStartTime)+"ms");
            }
            ...
            for (var i = 0, l = c.length; i < l; i++) {
                ...
                if(String(m.cellHtml).indexOf("div")!=-1){
        ...
        if(b!=1 && exportMap[that.uid] && exportMap[that.uid].startSum){
            ...
            for (var i = 0,srs = that.summaryRows; i < srs; i++) {
                ...
                for (var n = 0, leg = a.length; n < leg; n++) {
                    ...
                    var m = that._OnDrawSummaryCell(v, item, null, i);
                    var obj = toTransformationData(m.cellHtml,that);
    ...
    mini.DataGrid.prototype.exportToExeclByType = function(options){
        ...
        if(that.showPager){
            //有分页
            exportMap[that.uid].exportStatus = "init";
            var total = that.getTotalCount();
            var maxPage = parseInt(total/options.pageSize);
            if(total%options.pageSize != 0){
                maxPage += 1;
            }
            exportMap[that.uid].currentIndex = 0;
            exportMap[that.uid].maxIndex = maxPage-1;
            mini.hideMessageBox(exportMap[that.uid].loadingId);
            exportMap[that.uid].loadingId = mini.loading("正在生成第"+(exportMap[that.uid].currentIndex+1)+"页数据,共"+(exportMap[that.uid].maxIndex+1)+"页");
            that.reload();
        }else{
    ...
    function inNotExist(column,uid){
        if(column.field){
            if(column.field == "action"){
                console.log(column,exportMap[uid].notExist);
            }
            if(exportMap[uid].notExist.contains(column.field+"")){
                return true;
            }
        }
        if(column.name){
            if(exportMap[uid].notExist.contains(column.name+"")){
                return true;
            }
        }
        return false;
